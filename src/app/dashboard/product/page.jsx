...
import {
  create_product__Request__API,
  read_all_product__Request__API,
} from "@/utility/api";
...
import { DeleteAlertProductPost } from "@/utility/DeleteAlert";
...
export default function Page() {
  ...
  const submit = (e) => {
    ...
    let des = editorData;
    create_product__Request__API({ title, imgCDN, des }).then((res) => {
      if (res) {
        SuccessToast("Product add successful!");
        titleRef.value = "";
        imgCDNRef.value = "";
        setEditorData("");
  ...
  const deleteProductControl = (id) => {
    DeleteAlertProductPost(id).then((res) => {
      if (res) {
        read_all_product__Request__API().then((res) => {
          setProduct(res.data);
        });
          ...
          <div className="grid ">
            <div className="p-5">
              <h1 className="text-xl font-semibold">
                Add Product - please fill in your information to continue
              </h1>
              <div className="mt-6">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex justify-end gap-4">
                        <span
                          ...
                          onClick={() => deleteProductControl(item.id)}
                        >
                          <svg
                        ...
                        <ProductUpdateDialog id={item.id} />
                      </div>
                    </td>
                  </tr>
                ))}
