...
import {
  read_single_product__Request__API,
  update_product__Request__API,
} from "@/utility/api";
import { SuccessToast } from "@/utility/FormHelper";

export function ProductUpdateDialog({ id }) {
  let titleRef,
    imgCDNRef = useRef();
  ...
  const handleOpen = () => {
    setOpen(!open);
    read_single_product__Request__API(id).then((res) => {
  ...
  const submit = (id) => {
    ...
    let des = editorData;
    update_product__Request__API({ title, imgCDN, des, id }).then((res) => {
      if (res) {
        handelClose();
